...
export function SalesForm({ initialData, open, setOpen }: SalesFormProps) {
  ...
  // useWatch (not the imperative watch()) — reliably re-renders on
  // useFieldArray-registered nested paths. Plain watch("items") was
  // observed to freeze at its initial value even while the DOM inputs
  // themselves updated correctly, which silently broke every derived
  // calculation below.
  ...
  return (
    <Sheet open={open} onOpenChange={(next) => !isBusy && setOpen(next)}>
      <SheetContent side="right" className="w-full overflow-y-auto md:min-w-lg p-6">
        <SheetHeader className="border-b p-0 pb-2">
