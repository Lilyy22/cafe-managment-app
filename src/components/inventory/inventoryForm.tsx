...
// Shape actually returned by the API/DB. unit_of_measurement is a nested
// object on READ ({ name: 'pcs' }), but the form submits it as a flat
// string on WRITE — same asymmetry as products' linked ingredient.
export interface InventoryRecord {
  ...
  remaining: number
  price: number
  unit_of_measurement: MeasurementUnit | null
}
...
function getDefaultValues(
  isEditing: boolean,
  initialData: InventoryRecord | undefined
): InventoryFormInput {
...
export function InventoryForm({ initialData, open, setOpen }: InventoryFormProps) {
  ...
  // Auto-fill "remaining" with "quantity" only for a brand-new purchase,
  // and only while the user hasn't touched "remaining" yet.
  useEffect(() => {
    if (!isEditing && quantity) {
      setValue("remaining", quantity, { shouldValidate: true })
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [quantity, isEditing])

  // raw is InventoryFormInput (strings); parsing through the schema here
  // gets us InventoryFormData (real numbers) without depending on
  // react-hook-form's multi-generic resolver typing, which isn't resolving
  // correctly with the installed RHF/zod versions.
  const onSubmit = (raw: InventoryFormInput) => {
    const data: InventoryFormData = inventorySchema.parse(raw)
    const fields = {
      item: data.item.trim(),
      quantity: data.quantity,
      remaining: data.remaining,
      price: data.price,
      unit_of_measurement: data.unit_of_measurement as unknown as MeasurementUnit,
    }
                    ...
                    >
                      <SelectTrigger id="unit_of_measurement" className="w-full" aria-invalid={!!errors.unit_of_measurement}>
                        <SelectValue placeholder="Select a unit">
                          {selectedUnit
                            ? `${selectedUnit.name} ${selectedUnit.description ? `(${selectedUnit.description})` : ""}`
                            : undefined}
                        </SelectValue>
                      </SelectTrigger>
                      <SelectContent>
